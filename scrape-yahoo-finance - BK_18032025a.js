...
async function fetchYahooFinanceData(ticker, attempt = 1) {
    try {
        ...
        let result;
        try {
            result = await yahooFinance.quoteSummary(ticker, { modules });
        } catch (fetchError) {
            await logError('Yahoo Finance fetch attempt failed', { 
                ticker, 
                attempt, 
                error: { message: fetchError.message, stack: fetchError.stack },
                nodeVersion: process.version,
                yahooFinanceVersion: require('yahoo-finance2/package.json').version
            });
            throw fetchError; // Let retry logic handle it
        }
        ...
        await logInfo('Received raw API response', { 
            ...
            // rawData: result // Uncomment for debugging
        });
        ...
        data.cash_date = null; // Not available
        ...
        data.debt_value = data.liabilities; // Alias
        ...
        data.trailing_pe = result.summaryDetail?.trailingPE || 
                          (result.defaultKeyStatistics?.trailingEps && result.price?.regularMarketPrice ? 
                           result.price.regularMarketPrice / result.defaultKeyStatistics.trailingEps : null) || 
                          null;
        ...
        data.gross_profit = result.financialData?.grossProfits || 
                           ...
                           null;
        data.operating_expense = incomeStmt.totalOperatingExpenses || null;
