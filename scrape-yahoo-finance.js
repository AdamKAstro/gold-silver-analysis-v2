...
async function fetchYahooFinanceData(ticker, attempt = 1) {
    try {
        ...
        data.liabilities = result.financialData?.totalDebt || (result.balanceSheetHistory?.balanceSheetStatements[0]?.totalLiab) || null;
        ...
        data.net_financial_assets = (data.cash_value && data.liabilities) ? (data.cash_value - data.liabilities) : null;
